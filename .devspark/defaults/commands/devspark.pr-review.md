...
_Note: APPROVE is blocked if Spec Lifecycle is not Complete or tasks are incomplete for feature branches._

## Action Items

_All findings ordered by severity. CRITICAL and HIGH items include broken code and the fix._
...
_Skip this section if there is nothing noteworthy. Maximum 5 bullets._
...
_When addressing these findings, run `/devspark.address-pr-review {PR_ID}`. The review file must be committed on its own — this rule is enforced by the prompt and can also be enforced by the optional pre-commit hook._
